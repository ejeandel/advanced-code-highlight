// heavily derivative of
// https://github.com/quarto-dev/quarto-cli/blob/main/src/resources/formats/revealjs/plugins/line-highlight/line-highlight.js

window.AdvancedCodeHighLight = function () {
  ...
  const kCodeLineNumbersAttr = "data-advanced-line-numbers";
  ...
  function initQuartoLineHighlight(deck) {
        ...
        if (handleLinesSelector(deck, codeLineAttr)) {
          ...
          codeBlock.forEach((code) => {
            ...
            const highlightSteps = splitLineNumbers(codeLineAttr);
			  ...
			  .forEach((span) => {
				  ...
				  span.setAttribute(
                      ...
                      highlightCodeBlock(fragmentBlock, step.lines);

                      fragmentBlock.setAttribute(kFragmentIndex, step.fragment - 1) ;
            ...
            if (slide) {
              ...
              code.setAttribute(kFragmentIndex, highlightSteps[0].fragment - 1);
              highlightCodeBlock(code,highlightSteps[0].lines);
          });
        }
      }
    });
    ...
    function highlightCodeBlock(codeBlock, lines) {
	if (lines == null)
	    return;
	codeBlock.classList.add("has-advanced-highlights");
        codeBlock.parentNode.classList.add("code-wrapper");
	
	lines.forEach((highlight) => {
  ...
  function splitLineNumbers(lineNumbersAttr) {
    ...
    // for each step, calculate first and last line, if any
	  ...
	  var fragments = highlights.split(delimiters.fragmentSpec);
	  ...
	  const fragment_number = fragments[0];
	  const lines = fragments[1].split(delimiters.line);
